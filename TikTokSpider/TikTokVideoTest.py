...
def GetWebdriver(ini):
    if ini.driver is None:
        ...
        #ini.driver = webdriver.Chrome(options=options, executable_path='Chrome/chromedriver.exe', desired_capabilities=ini.caps)
        s = Service(executable_path='Chrome/chromedriver.exe')
        ini.driver = webdriver.Chrome(service=s, options=options, desired_capabilities=ini.caps)
...
# 抓取单个作品script[id="RENDER_DATA"]的数据
def GetFrontPageData2(ini):
    print('*****************  正在处理作品数据，请勿操作  *********************')
    soup = BeautifulSoup(ini.driver.page_source, 'html.parser')
    response = soup.select('script[id="RENDER_DATA"]')[0].text
    response = urllib.parse.unquote(response)
...
def DownloadVideo(ini, video, title):
    ...
    if os.path.exists(f'{ini.path}/{title}.mp4'):
        ...
        return fileSize
    # 请求视频
    response = requests.get(video, headers=ini.header, verify=False)
...
def DownloadImageOrMusic(ini, image_url, imageAlbumTitle, title):
    ...
    # 请求文件
    print('正在下载：' + title + '文件')
    response = requests.get(image_url, headers=ini.header, verify=False)
    fileSize = len(response.content)
...
def Home(ini, *url):
    ...
    if ini.userAvatarDownload:
        userAvatarURL = 'http:' + data[key]["user"]["user"]["avatar300Url"]
    ...
    for aweme in aweme_list:
        ...
        try:
            ...
            if 'url' in aweme['download']:
                # 构造视频链接
                video_url = 'http:' + aweme['video']['playAddr'][0]['src']
...
# 下载单个作品
def Single(ini, *url):
    ini.url = url[0] if len(url) else input('请输入单个作品链接：')
    regex = r"(?i)\b((?:https?://|www\d{0,3}[.]|[a-z0-9.\-]+[.][a-z]{2,4}/)(?:[^\s()<>]+|\(([^\s()<>]+|(\([^\s()<>]+\)))*\))+(?:\(([^\s()<>]+|(\([^\s()<>]+\)))*\)|[^\s`!()\[\]{};:'\".,<>?«»“”‘’]))"
    ini.url = re.findall(regex, ini.url)[0][0]
    ...
    # 处理首页数据
    data = GetFrontPageData2(ini)

    # 数据节点的这个数字会变，所以直接查
    key = [a for a in data if 'awemeId' in data[a]][0]
